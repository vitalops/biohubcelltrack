...
BASE = 'kernels/submit-c/biohub-submit-c.ipynb'
OURS = 'kernels/submit-lf/biohub-submit-full.ipynb'
OUT = 'kernels/submit-c/biohub-submit-c.ipynb'

ours = json.load(open(OURS)); base = json.load(open(BASE))
o = ''.join(ours['cells'][5]['source'])
bi = [i for i, c in enumerate(base['cells']) if 'load_divnet' in ''.join(c['source'])]
assert len(bi) == 1, bi
...
meta = json.load(open('kernels/submit-c/kernel-metadata.json'))
...
json.dump(meta, open('kernels/submit-c/kernel-metadata.json', 'w'), indent=2)
